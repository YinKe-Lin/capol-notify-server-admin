...
public class MessageProducer {
    ...
    public void sendWeChatMsg(WeChatMsgCommand weChatMsgCommand) {
        UserQueueDTO userQueueDTO = queueService.getUserQueueByIdAndType(weChatMsgCommand.getUserId(), weChatMsgCommand.getBusinessType());
        if (userQueueDTO != null) {
            messagePublisher.messageSender(weChatMsgCommand, userQueueDTO.getQueue(), weChatMsgCommand.getPriority(), weChatMsgCommand.getMessageId().toString());
        } else {
            log.error("微信消息发送失败, 用户:{} 业务类型:{} 配置的队列不存在或已经被禁用!", weChatMsgCommand.getUserId(), weChatMsgCommand.getBusinessType());
            throw new DomainException(String.format("微信消息发送失败, 用户:%s 业务类型:%s 配置的队列不存在!", weChatMsgCommand.getUserId(), weChatMsgCommand.getBusinessType()), EnumExceptionCode.BadRequest);
        }
    }
    ...
    public void sendDingDingGroupMsg(DingDingGroupMsgCommand groupMsgCommand) {
        ...
        if (userQueueDTO != null) {
            ...
            log.error("钉钉群组消息发送失败, 用户:{} 业务类型:{} 配置的队列不存在或已经被禁用!", groupMsgCommand.getUserId(), groupMsgCommand.getBusinessType());
            throw new DomainException(String.format("钉钉群组消息发送失败, 用户:%s 业务类型:%s 配置的队列不存在!", groupMsgCommand.getUserId(), groupMsgCommand.getBusinessType()), EnumExceptionCode.BadRequest);
        }
    }
    ...
    public void sendEmailMsg(EmailContentCommand emailContentCommand) {
        UserQueueDTO userQueueDTO = queueService.getUserQueueByIdAndType(emailContentCommand.getUserId(), emailContentCommand.getBusinessType());
        if (userQueueDTO != null) {
            messagePublisher.messageSender(emailContentCommand, userQueueDTO.getQueue(), emailContentCommand.getPriority(), emailContentCommand.getMessageId().toString());
        } else {
            log.error("邮件消息发送失败, 用户:{} 业务类型:{} 配置的队列不存在或已经被禁用!", emailContentCommand.getUserId(), emailContentCommand.getBusinessType());
            throw new DomainException(String.format("邮件消息发送失败, 用户:%s 业务类型:%s 配置的队列不存在!", emailContentCommand.getUserId(), emailContentCommand.getBusinessType()), EnumExceptionCode.BadRequest);
        }
    }
    ...
    public void sendSMSMsg(SMSMsgCommand smsMsgCommand) {
        ...
        if (userQueueDTO != null) {
            messagePublisher.messageSender(smsMsgCommand, userQueueDTO.getQueue(), smsMsgCommand.getPriority(), smsMsgCommand.getMessageId().toString());
        } else {
            log.error("SMS消息发送失败, 用户:{} 业务类型:{} 配置的队列不存在或已经被禁用!", smsMsgCommand.getUserId(), smsMsgCommand.getBusinessType());
            throw new DomainException(String.format("SMS件消息发送失败, 用户:%s 业务类型:%s 配置的队列不存在!", smsMsgCommand.getUserId(), smsMsgCommand.getBusinessType()), EnumExceptionCode.BadRequest);
        }
    }
}
