...
public class MQMessageSaveAspect {
    ...
    private void processMessageContent(Long messageId, Integer priority, String userId, EnumMessageType messageType, String messageBusinessType, EnumProcessStatusType processStatusType, Object message) {
        ...
        if (userQueueDTO == null) {
            log.error("切面记录消息失败, 指定的用户：{} 业务类型：{}的队列不存在或已经被禁用!", userId, messageBusinessType);
            throw new DomainException(String.format("切面记录消息失败, 指定的用户：%s 业务类型：%s 的队列不存在或已经被禁用!", userId, messageBusinessType), EnumExceptionCode.InternalServerError);
        }
