...
public class AuthenticationService {
    ...
    public UserDescriptor authenticateByPassword(String account, String password) {
        ...
        if (user == null) {
            throw new DomainException("账户名称不存在!", EnumExceptionCode.UserNotExists);
        }
        if (StringUtils.isBlank(user.getPassword())) {
            throw new DomainException("账户密码为空, 需要设置密码才可使用!", EnumExceptionCode.IllegalAccountOrPassword);
        }
        if (user.getDisabled()) {
            throw new DomainException("账户已经被禁用!", EnumExceptionCode.Forbidden);
        }
